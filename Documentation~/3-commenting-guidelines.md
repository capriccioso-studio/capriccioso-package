...
We document all parameters with `<param>` tags. The description should explain what the parameter is used for.


## Same object, same comment

When the same object appears in multiple places (e.g., a parameter that matches a field), we maintain consistent documentation.


## Values and literals

When documenting values and literals in comments, use `<c>` tags for inline code references.

**Example:**
```csharp
/// <summary>
/// The default timeout is <c>30000</c> milliseconds.
/// </summary>
```
